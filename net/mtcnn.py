...
class mtcnn():
    ...
    def detectFace(self, img, threshold):
        ...
        for scale in scales:
            ...
            scale_img = cv2.resize(copy_img, (ws, hs), interpolation=cv2.INTER_LINEAR)
            inputs = np.expand_dims(scale_img, 0)
            output = self.Pnet.predict(inputs, verbose=0)
            #---------------------------------------------#
            #   每次选取图像金字塔中的一张图片进行预测
            #   预测结果也是一张图片的，
        ...
        for rectangle in rectangles:
            ...
            #-----------------------------------------------#
            scale_img = cv2.resize(crop_img, (24, 24), interpolation=cv2.INTER_LINEAR)
            predict_24_batch.append(scale_img)

        cls_prob, roi_prob = self.Rnet.predict(np.array(predict_24_batch), verbose=0)
        #-------------------------------------#
        #   解码的过程
        #-------------------------------------#
        ...
        cls_prob, roi_prob, pts_prob = self.Onet.predict(np.array(predict_batch),verbose=0)
